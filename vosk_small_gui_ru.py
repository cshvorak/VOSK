...
def clear_log():
	...
	with open('log.txt', 'w') as log_clear:
			log_clear.write('')
			log_clear.close()
			label_last_string.config(text='Лог очищен')
...
def about_window():
	...
	label_about = Label(about, background='black', height=10, foreground='green', font=('Arial', 10), text='VOSK API транскрибатор\
		\n\nVersion: 1.0 alpha\n\n(c) Константин Шворак\n\n2024')
...
filemenu.add_command(label="Скопировать в буфер обмена", command=paper_clip)
filemenu.add_command(label="Открыть лог", command=open_log)
filemenu.add_command(label="Очистить лог", command=clear_log)
filemenu.add_command(label="Выход", command=terminate_program)
helpmenu = Menu(mainmenu, tearoff=0)
helpmenu.add_command(label="О программе...", command=about_window)
...
label_datetime = Label(root, height=2, background='black', foreground='green', font=('Arial', 10),
	text=('Запись начата: ' + str(current_datetime) + '\nПоследняя реплика:'))
...
button_paperclip = Button(root, height=2, text='Скопировать в буфер обмена', state='normal', command=paper_clip)
